...
```shell
npm install mjs-schema
```

Build
-----

To build the browser version you will need node.js and the development dependencies that can be installed with npm (type `npm install` in the project directory).
...
Include mjs-schema in your project with:

```js
import schema from 'mjs-schema';
```
...
reading a schema description is quite easy even if you are not familiar with mjs-schema.
...
JSON Schemas generated by mjs-schema itself.
...
starting with the first. If there's a match, mjs-schema first resolves the sub-patterns, and then
...
mjs-schema sees in fact `{ left : [ Number, undefined ], right : [ Number, undefined ] }` as first
...
'anything else' in an 'or' relation. If mjs-schema sees `undefined` in an or relation, it assumes
...
Better JSON Schema support. mjs-schema should be able to parse any valid JSON schema and generate
...
Feel free to open an issue or send a pull request if you would like to help improving mjs-schema or find a bug.
