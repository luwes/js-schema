(function (global, factory) {
  ...
  Schema.prototype = {
    ...
    toJSON: session(function(makeReference) {
      ...
      if (!session.serialized)
        session.serialized = { objects: [], jsons: [], ids: [] };
  ...
  var schema = function(schemaDescription) {
    ...
    if (json.additionalProperties !== undefined) {
      other =
        json.additionalProperties === false
          ? instance
          : Schema.fromJSON(json.additionalProperties);
    }
    ...
    toJSON: Schema.session(function() {
      ...
      if (onlyEquality) {
        ...
        json['type'] = subjsons.map(function(json) {
          var simpleType =
            typeof json.type === 'string' && Object.keys(json).length === 1;
  ...
  Schema.fromJSON.def(function(sch) {
    ...
    if ('pattern' in sch) {
      ...
      return new RegexpSchema(
        RegExp('^.{' + [sch.minLength || 0, sch.maxLength].join(',') + '}$')
      );
    } else {
      return new RegexpSchema();
    }
  ...
  Array.of = function() {
    ...
    if (args.length == 3) {
      if (!(typeof args[2] === 'number')) {
        throw new Error(
          '3 arguments were passed to Array.of and 1st argument (minLength) SHOULD be number NOT ' +
            args[2]
        );
      }
      if (!(typeof args[1] === 'number')) {
        throw new Error(
          '3 arguments were passed to Array.of and 2nd argument (maxLength) SHOULD be number NOT ' +
            args[1]
        );
      }
    }
    if (args.length === 2) {
      if (!(typeof args[1] === 'number')) {
        throw new Error(
          '2 arguments were passed to Array.of and 1nd argument (length) SHOULD be number NOT ' +
            args[1]
        );
      }
      args[2] = args[1];
  ...
  Schema.fromJSON.def(function(sch) {
    ...
    if (!session.deserialized)
      session.deserialized = { references: {}, subscribers: {} };
    ...
    if (sch.id != null) {
      ...
      if (session.deserialized.references[id])
        return session.deserialized.references[id];

      // If not, returning a reference, and when the schema gets known, resolving the reference
      if (!session.deserialized.subscribers[id])
        session.deserialized.subscribers[id] = [];
      var reference = new SchemaReference();
      session.deserialized.subscribers[id].push(
        reference.resolve.bind(reference)
      );
